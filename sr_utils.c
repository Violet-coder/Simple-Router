...
uint32_t icmp_cksum (sr_icmp_hdr_t *icmp_hdr, int len) {
    uint16_t curr_cksum, calc_cksum;
    curr_cksum = icmp_hdr->icmp_sum;
    icmp_hdr->icmp_sum = 0;
    calc_cksum = cksum(icmp_hdr, len);
    icmp_hdr->icmp_sum = curr_cksum;
    return calc_cksum;
}
...
void ip_int_to_string(char* buffer, uint32_t ip){
    sprintf(
            buffer,
            "%d.%d.%d.%d",
            ip >> 24,
...
/*find the entry in the routing table that has the longes prefix match*/
struct sr_rt* longest_prefix_match(struct sr_instance* sr, uint32_t ip) {
    /*init the longest prefix match entry*/
    struct sr_rt* longest_prefix_match_entry = NULL;
    char ip_addr[15];

    ip_int_to_string(ip_addr, ntohl(ip));
    fprintf(stderr, "Finding longest prefix for %s ...\n", ip_addr);

    struct sr_rt* rt_walker = sr->routing_table;
    while(rt_walker) {
        /*go through all the entries of the routing table*/
        if((rt_walker->dest.s_addr & rt_walker->mask.s_addr) == (ip & rt_walker->mask.s_addr)) {
            /* check whether to update the longest_prefix_match_entry, if there is longer prefix, update the  longest prefix match entry*/
            if(!longest_prefix_match_entry || rt_walker->mask.s_addr > longest_prefix_match_entry->mask.s_addr) {
                longest_prefix_match_entry = rt_walker;
            }
        }
        rt_walker = rt_walker->next;
    }

    /*if there is longest_prefix_match_entry, get */
    if(longest_prefix_match_entry) {
       printf("Longest prefix match entry found.\n");
    } else {
        printf("Cannot find any longest matching prefix.\n");
    }

    return longest_prefix_match_entry;
}
